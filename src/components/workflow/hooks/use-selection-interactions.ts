...
export const useSelectionInteractions = () => {
    const store = useStoreApi() // reactflow的node,edge
        ...
        const newNodes = produce(nodes, (draft) => {
          draft.forEach((node) => {
            if (node.data._isBundled)
              node.data._isBundled = false // 未圈中
          })
        })
        setNodes(newNodes)
        const newEdges = produce(edges, (draft) => {
          draft.forEach((edge) => {
            if (edge.data._isBundled)
              edge.data._isBundled = false // 未圈中
          })
        })
        setEdges(newEdges)
      }
    ...
    const handleSelectionChange = useCallback<OnSelectionChangeFunc>(({ nodes: nodesInSelection, edges: edgesInSelection }) => {
      ...
      const newNodes = produce(nodes, (draft) => {
        draft.forEach((node) => {
          ...
          if (nodeInSelection)
            node.data._isBundled = true // 圈中
          else
            node.data._isBundled = false // 圈中
        })
      })
      setNodes(newNodes)
      const newEdges = produce(edges, (draft) => {
    ...
    const handleSelectionDrag = useCallback((_: MouseEvent, nodesWithDrag: Node[]) => {
      ...
      const newNodes = produce(nodes, (draft) => {
        draft.forEach((node) => {
          ...
          if (dragNode)
            node.position = dragNode.position // 移位置
        })
      })
      setNodes(newNodes)
    }, [store, workflowStore])
    ...
    // deprecated
    // 这个方法是在点击切换成手抓模式时取消图中选择框，以后shift长按就是指针模式 todo
