...
import { Node, WorkflowRunningStatus } from '../types';
// import {
//   useStore as useZustandStore,
// } from 'zustand'
import { fetchWorkflowDraft, syncWorkflowDraft } from '@/services/workflow';
...
import { Connection, getIncomers, getOutgoers, useStoreApi } from 'reactflow';
  ...
  const isValidConnection = useCallback(
    ({ source, sourceHandle, target }: Connection) => {
      ...
      if (sourceNode && targetNode) {
        const sourceNodeAvailableNextNodes = NODES_EXTRA_INFO[
          sourceNode.data.type
        ].getValidNextNodes(chatMode, !!sourceNode.parentId); // availableNextNodes是选择器里能选的元素
        const targetNodeAvailablePrevNodes = NODES_EXTRA_INFO[
          sourceNode.data.type
        ].getValidPrevNodes(chatMode, !!targetNode.parentId);
...
export const useNodesReadOnly = () => {
  ...
  const workflowRunningData = useWorkflowDraftStore(
    (s) => s.workflowRunningData,
  );
  ...
  const getNodesReadOnly = useCallback(() => {
    const { workflowRunningData, readonly } = workflowStore.getState();
    return (
      readonly ||
...
export const useGetNode = () => {
  const store = useStoreApi();
  const { getNodes } = store.getState();
  const func = useCallback((nodeId: string) => {
    const allNodes = getNodes();
