    ...
    <div className='mb-1 px-3 py-1'>
      <div className='space-y-0.5'>
        {variables.map(variable => (
          <div key={variable.name} className='flex items-center h-6 justify-between bg-gray-100 rounded-md  px-1 space-x-1 text-xs font-normal text-gray-700'>
            <div className='w-0 grow flex items-center space-x-1'>
              {/* <Variable02 className='shrink-0 w-3.5 h-3.5 text-primary-500' /> */}
              <span className='w-0 grow truncate text-xs font-normal text-gray-700'>{variable.name}</span>
            </div>

            <div className='ml-1 flex items-center space-x-1'>
              {variable.required && <span className='text-xs font-normal text-gray-500 uppercase'>必填</span>}
